...
// ✅ FIXED: Only import used Web3Auth hooks
import { 
  useWeb3AuthConnect, 
  useWeb3AuthUser 
} from "@web3auth/modal/react";

// ✅ FIXED: Only import used Solana hooks
import { 
  useSolanaWallet, 
...
// ✅ FIXED: Only import used types from @solana/web3.js
...
// Type safety for Web3Auth accounts
type Web3AuthAccounts = string[] | null | undefined;
...
const AddLiquidityModal: React.FC<AddLiquidityModalProps> = ({ 
  ...
  // ✅ FIXED: Web3Auth hooks - only what we use
  const { isConnected: web3AuthConnected } = useWeb3AuthConnect();
  const { userInfo } = useWeb3AuthUser();
  
  // ✅ FIXED: Solana-specific hooks with proper types
  const { 
    solanaWallet, 
    accounts: web3AuthAccounts, 
    connection: web3AuthConnection 
  } = useSolanaWallet() as {
    solanaWallet: unknown; // ✅ FIXED: Changed from 'any' to 'unknown'
    accounts: Web3AuthAccounts;
    connection: Connection | null;
  };
  
  // ✅ FIXED: Only destructure what we actually use
  const { 
    signAndSendTransaction: web3AuthSignAndSend, 
    loading: signAndSendLoading, 
    error: signAndSendError
  } = useSignAndSendTransaction();
  ...
  // ✅ ENHANCED: Improved wallet detection with official Web3Auth patterns
  const walletInfo = useMemo(() => {
    ...
    // ✅ FIXED: Web3Auth detection with comprehensive null checks
    if (web3AuthConnected && 
        web3AuthAccounts && 
        Array.isArray(web3AuthAccounts) && 
        web3AuthAccounts.length > 0 && 
        solanaWallet) {
      try {
        const publicKey = new PublicKey(web3AuthAccounts[0]);
        console.log('✅ Web3Auth wallet detected:', {
          publicKey: publicKey.toBase58(),
          hasConnection: !!web3AuthConnection,
          canSign: !!web3AuthSignAndSend,
    ...
    console.log('❌ No wallet connected', {
      ...
      web3AuthConnected,
      accountsLength: web3AuthAccounts ? web3AuthAccounts.length : 0,
      accountsIsArray: Array.isArray(web3AuthAccounts),
      solanaWallet: !!solanaWallet
    });
    ...
    // ✅ FIXED: Removed 'userInfo' from dependencies - it's not used in the calculation
    traditionalConnected, 
  ...
  }, [walletInfo, signAndSendLoading, signAndSendError]);
  ...
  const fetchUserTokenBalance = useCallback(async () => {
    ...
    try {
      ...
      // Extract the token symbol from pool name
      const { tokenX } = getTokenNames();
  ...
  // ✅ ENHANCED: Transaction handler with official Web3Auth patterns
  const handleAddLiquidity = async () => {
    ...
    if (!walletInfo.canTransact) {
      const message = walletInfo.type === 'web3auth' 
        ? 'Web3Auth wallet is not ready for transactions. Please try reconnecting.'
        : 'Wallet is not ready for transactions.';
      showToast.error('Cannot Transact', message);
      return;
    }
    ...
    // Check for ongoing Web3Auth transaction
    ...
    try {
      ...
      // ✅ ENHANCED: Handle transactions based on wallet type with official patterns
      if (walletInfo.type === 'traditional') {
        ...
        console.log('🔐 Processing with Web3Auth using official pattern...');
        
        if (!web3AuthConnection || !web3AuthSignAndSend) {
          throw new Error('Web3Auth connection or signing function not available');
        }
        
        // ✅ OFFICIAL PATTERN: Handle both single and array transactions
        if (Array.isArray(result.transaction)) {
          for (const tx of result.transaction) {
            // Cast to the official type
            const transaction = tx as TransactionOrVersionedTransaction;
            
            try {
              const signature = await web3AuthSignAndSend(transaction);
              console.log('✅ Web3Auth transaction signature:', signature);
              
              // Wait for confirmation using the Web3Auth connection
          ...
          const transaction = result.transaction as TransactionOrVersionedTransaction;
      ...
      // ✅ ENHANCED: Better error handling for Web3Auth
      let errorMessage = 'Unknown error';
      
      if (error instanceof Error) {
        errorMessage = error.message;
        
        // Handle Web3Auth specific errors
  ...
  // ✅ Debug component for Web3Auth hooks
  const Web3AuthDebugInfo = () => {
    if (process.env.NODE_ENV !== 'development') return null;

    return (
      <div className="fixed bottom-4 left-4 bg-black/80 text-white p-4 rounded text-xs z-50 max-w-sm">
        <div className="font-bold mb-2">Web3Auth Debug Info:</div>
        <div>Connected: {web3AuthConnected ? '✅' : '❌'}</div>
        <div>Accounts: {web3AuthAccounts && Array.isArray(web3AuthAccounts) ? web3AuthAccounts.length : 0}</div>
        <div>First Account: {web3AuthAccounts && web3AuthAccounts.length > 0 ? web3AuthAccounts[0].slice(0, 8) + '...' : 'N/A'}</div>
        <div>Solana Wallet: {solanaWallet ? '✅' : '❌'}</div>
        <div>Connection: {web3AuthConnection ? '✅' : '❌'}</div>
        <div>Can Sign: {typeof web3AuthSignAndSend === 'function' ? '✅' : '❌'}</div>
        <div>Loading: {signAndSendLoading ? '⏳' : '✅'}</div>
        <div>Error: {signAndSendError ? '❌' : '✅'}</div>
            ...
            <DialogDescription className="text-sm text-sub-text">
              ...
              {process.env.NODE_ENV === 'development' && (
                <div className="mt-2 text-xs text-yellow-400">
                  Debug: {walletInfo.walletName} - {walletInfo.isConnected ? 'Connected' : 'Disconnected'} - {walletInfo.canTransact ? 'Can Transact' : 'Cannot Transact'}
                </div>
              )}
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter className="mt-8 flex flex-col gap-3 sm:flex-row">
            <Button 
              ...
              disabled={
                ...
                parseFloat(amount) <= 0 || 
                isLoading || 
                signAndSendLoading || // ✅ Add Web3Auth loading state
                isCheckingBalance ||
                isLoadingBins ||
